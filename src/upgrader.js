  ...
  var RULESET = [
    {
      ...
      description: "Inspect HTML for references to a recognized BootstrapCDN version of Bootstrap and swap it out for Bootstrap 3.1.1",
      run: function(doc) {
        var count = 0;
        
        // Replace Bootstrap CDN CSS with 3.1.1 version
        $(doc).find("link[rel=stylesheet][href]").each(function() {
          ...
          if ( href && href.match(new RegExp("//netdna.bootstrapcdn.com/twitter-bootstrap/2.3.[0-2]/css/bootstrap-combined.(no-icons\.)?min.css")) ) {
            $link.attr("href","//netdna.bootstrapcdn.com/bootstrap/3.1.1/css/bootstrap.min.css");
            count++;
          }
        });
          
        // Replace Bootstrap CDN JS with 3.1.1 version
        $(doc).find("script[src]").each(function() {
          ...
          if ( src && src.match(new RegExp("//netdna.bootstrapcdn.com/twitter-bootstrap/2.3.[0-2]/js/bootstrap.min.js")) ) {
            $script.attr("href","http://netdna.bootstrapcdn.com/bootstrap/3.1.1/js/bootstrap.min.js");
            count++;
          }
        });
    ...
    
    {
      ...
      run: function (doc) {
		...
		if (text != undefined)
		$(this).attr('class',text.replace('control-group', 'form-group'));
		count+=1;
		});
...

    {
      ...
      description: "Changes <code>.label</code> to <code>.label.label-default</code> and <code>.label-danger</code> to <code>.label-important</code>",
      run: function(doc) {
        var count = 0;
        var types = [];
        ...
        var dangeritems = $(doc).find(".label.label-important");
        count += dangeritems.addClass("label-danger").removeClass("label-important");
